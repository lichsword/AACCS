...
public class TreeModePanel extends AbstractModelPanel {
	...
	class ParseJSONAsTreeThread extends Thread {
		...
		private void parseJSONAsTree(final IconNode parentNode, final String childKeyString,
				final Object childValueObject) {
			try {
				if (childValueObject instanceof Integer) {
					IconNode node = factory.createNode(eJSONType.INT, childKeyString, "" + childValueObject);
					parentNode.add(node);
				} else if (childValueObject instanceof String) {
					String valueString = (String) childValueObject;
					if (isJsonArray(valueString)) {
						IconNode node = factory.createNode(eJSONType.ARRAY, childKeyString, "");
						parentNode.add(node);
						// handler child
						JSONArray array = JSONArray.parseArray(valueString);
						if (null != array) {
						...
						IconNode node = factory.createNode(eJSONType.OBJECT, childKeyString, "");
						parentNode.add(node);
						// handler child
						JSONObject object = JSONObject.parseObject(valueString);
						if (null != object) {
						...
						IconNode node = factory.createNode(eJSONType.STRING, childKeyString, valueString);
						parentNode.add(node);
					}
	...
	private void initContentView() {
		...
		rootNode = JSONNodeFactory.getInstance().createNode(eJSONType.OBJECT, "ROOT", "");
		...
		ImageIcon icon = ImageCachedManager.getInstance().getImageIconRes("icon_array.gif");
