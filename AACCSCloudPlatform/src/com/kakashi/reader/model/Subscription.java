...
public class Subscription implements Serializable, BaseColumns, Parcelable {
	...
	private final String TAG = Subscription.class.getSimpleName();

	/**
	 * 
	 */
	private static final long serialVersionUID = 4025602609461801310L;

	public static final String TABLE_NAME = "subscription";

	public static final Uri CONTENT_URI = Uri
			.parse(ReaderProvider.SUB_CONTENT_URI_NAME);
	...
	public static final String[] DEFAULT_SELECT = { _ID, _UID, _SORTID, _TITLE,
			_HTML_URL, _ICON, _UNREAD_COUNT, _NEWEST_ITEM_TIME, _READ_ITEM_ID,
			_SYNC_TIME, _ITEM_SYNC_TIME };
	...
	public static final String[] SELECT_SUM_UNREAD_COUNT = { "sum("
			+ _UNREAD_COUNT + ")" };

	public static final String SQL_CREATE_TABLE = "create table if not exists "
			+ TABLE_NAME + " (" + _ID + " integer primary key, " + _UID
			+ " text not null," + _SORTID + " text not null," + _TITLE
			+ " text not null," + _HTML_URL + " text," + _ICON + " blob,"
			+ _UNREAD_COUNT + " integer not null default 0,"
			+ _NEWEST_ITEM_TIME + " integer not null default 0,"
			+ _READ_ITEM_ID + " integer not null default 0," + _SYNC_TIME
			+ " integer not null default 0," + _ITEM_SYNC_TIME
			+ " integer not null default 0" + ")";

	public static final String[][] INDEX_COLUMNS = { { _UID },
			{ _UNREAD_COUNT }, { _NEWEST_ITEM_TIME }, { _ITEM_SYNC_TIME },
	...
	public Bitmap getIcon(Context context) {
		...
		try {
			...
			Log.e(TAG, "OutOfMemoryError...Just display no icon");
		} finally {
			csr.close();
		}
		return null;
	...
	public ContentValues toContentValues() {
		 ...
		 * values.put(_READ_ITEM_ID, this.readItemId); values.put(_SYNC_TIME,
		 * this.syncTime); values.put(_ITEM_SYNC_TIME, this.itemSyncTime);
		 */
		return values;
	}
	...
	public CharSequence toLabelWithUnread() {
		return toLabelWithUnread(this.getTitle(), this.getUnreadCount());
	}

	private CharSequence toLabelWithUnread(CharSequence t, int unreadCount) {
		if (unreadCount == 0) {
			return t;
		}
