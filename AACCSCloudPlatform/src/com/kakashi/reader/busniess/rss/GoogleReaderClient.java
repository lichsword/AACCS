...
public class GoogleReaderClient extends ReaderClient {
    ...
    private static final String SPLIT_AUTH = "Auth";
    private static final String SPLIT_SID = "SID";
    ...
    private String initAuth() throws IOException, ReaderException {
        ...
        params.add(new BasicNameValuePair("source", "kakashi-reader-1.0"));
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
            	...
            	if (temp[0].equals(SPLIT_AUTH)) {
					this.auth = temp[1];
				}else if (temp[0].equals(SPLIT_SID)){
					this.sid = temp[1];
				}// end if
            }
            if (this.auth == null) {
    ...
    private static class JsonSubListHandler extends JsonHandlerAdapter {
        ...
        public boolean primitive(Object value)
            ...
            if (key.equals("subscriptions/id")) {
                this.sub.setUid(ReaderUtil.asString(value));
            } else if (key.equals("subscriptions/title")) {
                this.sub.setTitle(ReaderUtil.asString(value));
            } else if (key.equals("subscriptions/sortid")) {
                this.sub.setSortid(ReaderUtil.asString(value));
            } else if (key.equals("subscriptions/htmlUrl")) {
                this.sub.setHtmlUrl(ReaderUtil.asString(value));
            } else if (key.equals("subscriptions/categories/id")) {
                this.sub.addCategory(ReaderUtil.asString(value));
            }
            return true;
        }
    }
    ...
    private static class JsonTagListHandler extends JsonHandlerAdapter {
        ...
        public boolean primitive(Object value)
            ...
            if (key.equals("tags/id")) {
                String uid = ReaderUtil.asString(value);
                this.tag.setUid(uid);
                ...
                this.tag.setSortid(ReaderUtil.asString(value));
            }
            return true;
        }
    }
    ...
    private static class JsonUnreadCountHandler extends JsonHandlerAdapter {
        ...
        public boolean primitive(Object value)
            ...
            if (key.equals("unreadcounts/id")) {
                this.uid = ReaderUtil.asString(value);
            } else if (key.equals("unreadcounts/count")) {
                this.count = ReaderUtil.asInt(value);
            } else if (key.equals("unreadcounts/newestItemTimestampUsec")) {
                this.newestItemTime = (long) (ReaderUtil.asLong(value) / 1000000);
            }
            return true;
        }
    }
    ...
    private static class JsonItemListHandler extends JsonHandlerAdapter {
        ...
        public boolean primitive(Object value)
            ...
            if (key.equals("items/id")) {
                this.item.setUid(ReaderUtil.asString(value));
            } else if (key.equals("items/title")) {
                this.item.setTitle(ReaderUtil.asString(value));
            } else if (key.equals("items/published")) {
                this.item.setPublishedTime(ReaderUtil.asLong(value));
            } else if (key.equals("items/updated")) {
                this.item.setUpdatedTime(ReaderUtil.asLong(value));
            } else if (key.equals("items/alternate/href")) {
                this.item.setLink(ReaderUtil.asString(value));
            } else if (key.equals("items/alternate/type")) {
                this.item.setContentType(ReaderUtil.asString(value));
            } else if (key.equals("items/content/content")) {
                this.item.setContent(ReaderUtil.asString(value));
            } else if (key.equals("items/summary/content")) {
                this.item.setContent(ReaderUtil.asString(value));
            } else if (key.equals("items/author")) {
                this.item.setAuthor(ReaderUtil.asString(value));
            } else if (key.equals("items/categories")) {
                String category = ReaderUtil.asString(value);
